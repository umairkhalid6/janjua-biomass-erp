...
**Fix:** Dockerfile gained a dedicated `migrate` stage (`npm install prisma@<version from package.json>`) whose complete `node_modules` is copied to `/migrate` in the runner, together with `prisma.config.ts` and `prisma/` (schema + migrations) — fully isolated from the app's standalone `node_modules` so no version clobbering. Verified locally by replicating the exact `/migrate` layout and running the exact command against the dev DB.
**Follow-up 1:** the `migrate` stage must `apk add openssl` *before* `npm install` — `@prisma/engines` detects the platform's OpenSSL at install time to pick the schema-engine binary; without it the pre-deploy died in ~3s with no output (Railway's deploy-log pane also drops stderr, and Prisma writes all progress/errors to stderr — pipe `2>&1`).
**Follow-up 2 (the real smoking gun):** Railway's `preDeployCommand` is **not executed through a shell** — `&&`, `cd`, and redirects are passed as literal argv. `node -e "…" && npx prisma migrate deploy` therefore ran only `node -e` (extra tokens ignored → exit 0 → deploy proceeded, migrations never applied, no error); `cd /migrate && node …` tried to exec `cd` as a binary (instant fail, no output); `echo … && node …` ran only `echo`. Fix: bake `/migrate/deploy.sh` into the image and set `preDeployCommand` to `/bin/sh /migrate/deploy.sh` — two argv tokens, no shell parsing needed. (Community-documented Railway behavior: wrap pre-deploy commands in `/bin/sh -c`.)
